...
def build_qubo(data, A):
    ...
    # Tạo danh sách điểm
    all_points = [d['id'] for d in depots] + [c['id'] for c in customers]

    # Khởi tạo biến QUBO
    x = {
        (depot_id, v, i, j): Binary(f'x_{depot_id}_{v}_{i}_{j}')
        for depot_id in [d['id'] for d in depots]
    ...
    H_obj = quicksum(
        distance_matrix[(i, j)] * x[depot_id, v, i, j]
        for depot_id in [d['id'] for d in depots]
        for v in range(vehicles_per_depot)
        for i in all_points
        for j in all_points
    ...
    H_depot = A * quicksum(
        (1 - quicksum(x[depot_id, v, depot_id, j]
                      for j in all_points if j != depot_id)) ** 2 +
        (1 - quicksum(x[depot_id, v, i, depot_id]
                      for i in all_points if i != depot_id)) ** 2
        for depot_id in [d['id'] for d in depots]
        for v in range(vehicles_per_depot)
    )
...
def check_constraints(best_sample, data, points):
    """Kiểm tra tính hợp lệ của nghiệm cho bài toán nhiều depot"""
    start_time = time.time()
    valid = True
    ...
    # Kiểm tra ràng buộc 1: Mỗi khách hàng được phục vụ đúng 1 lần
    # for customer in customers:
    #     total_visits = sum(
    #         best_sample.get(f'x_{depot_id}_{v}_{i}_{customer}', 0)
    #         for depot_id in [d['id'] for d in depots]
    #         for v in range(vehicles_per_depot)
    #         for i in points if i != customer
    #     )
    #     if total_visits != 1:
    #         print(f"Khách hàng {customer} được phục vụ {total_visits} lần!")
    #         valid = False

    # Kiểm tra ràng buộc 2: Xe xuất phát và kết thúc tại depot của nó
    for depot in depots:
        depot_id = depot['id']
        for v in range(vehicles_per_depot):
            # Kiểm tra xe xuất phát từ depot
            start = sum(
                best_sample.get(f'x_{depot_id}_{v}_{depot_id}_{j}', 0)
                for j in points if j != depot_id
            )

            # Kiểm tra xe quay về depot
            end = sum(
                best_sample.get(f'x_{depot_id}_{v}_{i}_{depot_id}', 0)
                for i in points if i != depot_id
            )

            if start != 1 or end != 1:
                print(f"Xe {v} từ depot {depot_id} vi phạm ràng buộc depot!")
                valid = False

    # Kiểm tra ràng buộc 3: Giới hạn công suất xe
    # for depot in depots:
    #     depot_id = depot['id']
    #     for v in range(vehicles_per_depot):
    #         # Tính tổng nhu cầu của khách hàng được phục vụ bởi xe này
    #         total_demand = sum(
    #             demand[j] * best_sample.get(f'x_{depot_id}_{v}_{i}_{j}', 0)
    #             for i in points
    #             for j in customers
    #             if i != j
    #         )
    ...
    # Kiểm tra ràng buộc 4: Mỗi khách hàng được phục vụ bởi một depot duy nhất
    # for customer in customers:
    #     depot_allocations = sum(
    #         sum(
    #             best_sample.get(f'x_{depot_id}_{v}_{depot_id}_{customer}', 0)
    #             for v in range(vehicles_per_depot)
    #         )
    #         for depot_id in [d['id'] for d in depots]
    #     )
    #
    #     if depot_allocations != 1:
    #         print(f"Khách hàng {customer} được phân bổ không đúng: {depot_allocations} depot!")
    #         valid = False
